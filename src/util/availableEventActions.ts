...
export interface IAvailableAction {
  ...
  provider: string;
  result: string;
  nextActionsCustomer: Array<customerActions>;
  nextActionsProvider: Array<providerActions>;
  archived: boolean;
}
export type IAvailableActions = Array<IAvailableAction>;
...
export const validateAction = (
  ...
  if (type === "customer") {
    isAllowed = availableAction[0].nextActionsCustomer.includes(
      nextAction as customerActions
    );
  } else {
    isAllowed = availableAction[0].nextActionsProvider.includes(
      nextAction as providerActions
    );
  }
  return isAllowed;
};
